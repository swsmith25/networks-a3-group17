...
class SwitchConnection(object):
    ...
    def WriteTableEntry(self, table_entry, is_modify=False, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = 1
        update = request.updates.add()
        if (table_entry.is_default_action or is_modify):
        ...
        if dry_run:
            ...
            try: 
                ...
                print ("The table enry insertion fails. See the below error message.")
                printGrpcError(e)
   ...
   # Needed to send if ack_timeout_ns > 0 
    ...
    def ReadfromSwitch(self, dry_run=False, **kwargs):
        request = p4runtime_pb2.StreamMessageRequest()
        if dry_run:
            print("Switch Response: ", request) 
        else: 
            self.requests_stream.put(request)
            for item in self.stream_msg_resp:
                return item
    
    def sendPktToSwitch(self, payload, metadata=None, dry_run=False, **kwargs):
        request = p4runtime_pb2.StreamMessageRequest()
        packet = request.packet
        packet.payload = payload
        if (metadata is not None):
            for i in range(len(metadata)):
                meta = packet.metadata.add()
                meta.metadata_id = i+1
                meta.value = (metadata[i]).to_bytes(4,'big')
            
        if dry_run:
            print("Request for sending pkt to the switch:", request)
        else:
            self.requests_stream.put(request)
    
    # def sendToSwitchMeta(self, payload, metadata, dry_run=False, **kwargs):
    #     request = p4runtime_pb2.StreamMessageRequest()
    #     packet = request.packet
    #     packet.payload = payload
